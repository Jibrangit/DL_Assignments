...
def exponent_and_exponent_sum(z):
    z = z - np.max(z)
    exponent = np.exp(z)
    exponent_sum = np.sum(np.exp(z), axis=0, keepdims=True)
    print('Exponent and its sum is: ', exponent[:, 1:4], exponent_sum[:, 1:4])
...
def forward_propagation(WEIGHTS, BIASES, X_TRAINING, Y_TRAINING, NO_OF_HIDDEN_LAYERS, LAMBDA):
    ...
    Y_HAT = softmax_function(Z['z' + str(NO_OF_HIDDEN_LAYERS + 1)]) + 0.00000001
