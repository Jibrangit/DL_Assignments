...
def backward_propagation(Y_HAT, Y_TRAINING, WEIGHTS, BIASES, H, NUM_LAYERS, LAMBDA):
    ...
    dJ_dYhat = np.true_divide(Y_TRAINING, (Y_HAT+0.000000001))
...
if __name__ == '__main__':
    ...
    REG_CONSTANT1 = 0.1                 #Lambda
    NUM_EPOCHS1 = 1

    Valid_weights, Valid_biases = Stochastic_Gradient_Descent(X_tr, Y_tr, LEARNING_RATE1, DECAY_LEARNING_RATE1, REG_CONSTANT1, NUM_EPOCHS1, BATCH_SIZE, NUM_HIDDEN_LAYERS + 2)
    ...
    y_hat = np.argmax(Y_HAT_T, axis=0)
    y = np.argmax(Y_testing_ENCODED, axis=0)
